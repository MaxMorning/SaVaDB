...
public interface JpaStatRepo  extends JpaRepository<StatisticEntity, StatisticEntityPK> {
    ...
    @Query(value = "select * from statistic where (region_id = ?1) and (datediff(stat_date, ?2) >= 0) and (datediff(stat_date, ?2) % ?4 = 0) and (datediff(stat_date, ?3) < 0) order by stat_date", nativeQuery = true)
